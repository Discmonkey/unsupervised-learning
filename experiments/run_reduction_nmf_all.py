...
def plt_all(y1, y2):
    ...
    x_range_fire = range(2, len(y2) + 2)
    # explained variance

    ax1.plot(x_range_ball, y1, label="Min Error")

    ax1.set_title("Basketball Representation by NMF")
    ax1.set_ylabel("RMS Reconstruction Error")
    ax1.legend()

    ax2.plot(x_range_fire, y2, label="Min Error")
    ax2.set_title("Fire Representation by NMF")
    ax2.legend()
...
if __name__ == '__main__':
    ...
    for k in range(2, bball_data.shape[1] + 1):
        best_model = nmf.train_nmf(bball_data, k)
        all_models_b.append(best_model)
        min_errors_b.append(nmf.get_reconstruction_error(best_model))
    ...
    for k in range(2, fire_data.shape[1] + 1):
        best_model = nmf.train_nmf(fire_data, k)
        all_models_f.append(best_model)
        min_errors_f.append(nmf.get_reconstruction_error(best_model))

    plt_all(min_errors_b, min_errors_f)

    best_basketball_model = int(input("choose bball model"))
    ...
    bball_x, bball_y = common.single_transform(bball_model, temp_x + .5), temp_y

    fire_model = all_models_f[best_fire_index]
    temp_x, temp_y = util.get_x_y(fire_data)
    fire_x, fire_y = common.single_transform(fire_model, temp_x + .5), temp_y
