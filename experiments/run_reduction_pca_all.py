...
if __name__ == '__main__':
    ...
    bball_values = get_lambdas(bball_data)

    fire_data = load_dataset("fire")

    fire_values = get_lambdas(fire_data)

    # representational strength
    bball_errors = run_single_experiment(bball_data, pca)
    fire_errors = run_single_experiment(fire_data, pca)

    plt_all(bball_values, fire_values, bball_errors, fire_errors)
    plt_total_representation(bball_values, fire_values)

    # train components based on previous results
    num_bball_features = int(input("Choose number bball features"))
    bball_model = pca.train(bball_data, num_components=num_bball_features)
    ...
    fire_model = pca.train(fire_data, num_components=num_fire_features)
